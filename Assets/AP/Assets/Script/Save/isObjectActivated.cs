...
public class isObjectActivated : MonoBehaviour {
    ...
    // Method to unload the object
    public bool UnloadObject()
    {
        if(objectToUnload != null)
        {
            objectToUnload.SetActive(false);
            return true; // Return true to indicate the method was successful
        }
        return false; // Return false if the object to unload was not set
    }
